...
create table if not exists deals (
  ...
  expert_name text default 'Специалист',
  description text,
  status text check (status in ('pending', 'offered', 'active', 'completed', 'cancelled')) default 'pending',
  -- Offer fields (set by expert)
  offer_price integer,
  offer_deadline text,
  offer_comment text,
  -- Escrow
...
-- Indexes
create index if not exists idx_deals_room_code on deals(room_code);
...
create policy "Allow all for demo" on reviews for all using (true) with check (true);
